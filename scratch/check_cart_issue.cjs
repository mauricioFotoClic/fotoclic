...
async function check() {
    ...
    for (const user of users) {
        ...
        // Check photos of photographer 394d208c-07b3-49aa-a04b-63630cb85bb7
        const { data: photos, error: photosError } = await supabase
            .from('photos')
            .select('id, title, price')
            .eq('photographer_id', '394d208c-07b3-49aa-a04b-63630cb85bb7');
            
        if (photosError) {
            console.error("Error fetching photos:", photosError);
        } else {
            console.log("\nPhotographer's photos (394d208c-07b3-49aa-a04b-63630cb85bb7):", photos);
        }
        }
    }
}
