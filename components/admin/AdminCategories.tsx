    ...
    const handleFormSubmit = async (data: { name: string; image_url: string; image_file?: File }) => {
        try {
            ...
            if (editingCategory) {
                ...
                const newCategory = await api.createCategory(finalData);
                setCategories(prev => [...prev, newCategory]); // Append new category
            }
            handleCloseModal();
        } catch (error) {
            console.error("Failed to save category", error);
            showToast("Ocorreu um erro ao salvar a categoria.", "error");
        }
    };
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            if (success) {
                ...
                showToast('Erro: A categoria não foi encontrada ou não pôde ser excluída.', 'error');
            }
        } catch (error) {
            console.error("Falha ao excluir categoria", error);
            showToast('Ocorreu um erro ao tentar excluir a categoria.', 'error');
        } finally {
