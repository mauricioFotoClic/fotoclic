...
interface CategoryFormProps {
    onSubmit: (data: { name: string; image_url: string; image_file?: File }) => void;
    onCancel: () => void;
    initialData: Category | null;
}

const CategoryForm: React.FC<CategoryFormProps> = ({ onSubmit, onCancel, initialData }) => {
    const [formData, setFormData] = useState<{ name: string; image_url: string; image_file?: File }>({
        name: '',
        image_url: '',
    });
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                ...
                setFormData(prev => ({ ...prev, image_url: result, image_file: file }));
