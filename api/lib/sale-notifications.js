// Helper centralizado para disparo de e-mails transacionais de vendas no FotoClic
// Envia para Comprador, Administrador (svalmauricio@gmail.com) e Fotógrafo(s)
      ...
      for (const photogId of Object.keys(photographerSalesMap)) {
        try {
          ...
          if (photogData?.email) {
            ...
            const netAmount = (info.total * 0.85).toFixed(2).replace('.', ','); // estimativa líquida
            const photogHtml = `
              <div style="font-family: sans-serif; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 8px; padding: 20px;">
