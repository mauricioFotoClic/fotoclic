...
const api = {
  ...
  getPhotographers: async (): Promise<PhotographerWithStats[]> => {
    ...
    // 2. Get stats manually (aggregation in JS is easier to maintain without complex SQL views for now)
    ...
    return users.map(u => {
      ...
      const commissionValue = userSales.reduce((sum: number, s: any) => sum + (s.commission || 0), 0); // Commission generated FOR THE SYSTEM (platform fees)? Or for the photographer?
      // "Comissão Gerada" usually means what the *Platform* earned from this photographer, OR what the photographer earned.
      // Based on typical Admin views, "Commission Generated" = Platform Revenue from this user.
      // But looking at "Commission Value" in types, it might be the photographer's earnings.
      // Let's assume based on context "Comissão Gerada" = Platform Fee (since admin cares about that). 
      // WAIT: The previous code had "commissionRate: 0.15".
      // Let's calculate BOTH to be safe, but usually "commissionValue" on a photographer struct implies THEIR earnings.
      // actually, looking at AdminPhotographers.tsx: <td ...>{user.commissionValue.toLocaleString...}</td>
      // If I am the admin, I want to see how much I made from them. 
      // However, usually "Earnings" is what they made.
      // Let's stick to: commissionValue = what is stored in 'commission' column of sales. 
      // In `sales` table, `commission` is the Platform Fee (calculated as price * rate).
      // So commissionValue = Total Platform Fees generated by this photographer.
      ...
      return {
        ...
        photoCount,
        salesCount,
        commissionValue, // Total Platform Fees
        commissionRate: 0.15, // Default, ideally fetch from settings
        likesCount,
        avgRating: 0, // Implement ratings later if needed
        reviewCount: 0,
