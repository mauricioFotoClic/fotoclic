...
• *Tipo de Conta:* 📸 Fotógrafo (Vendedor de Fotos)
• *Nome:* ${name}
• *E-mail:* ${email}
...
• *Tipo de Conta:* 🎪 Produtor de Eventos / Organizador
• *Nome:* ${name}
...
• *Tipo de Conta:* 👤 Cliente (Comprador)
• *Nome:* ${name}
• *E-mail:* ${email}
• *Telefone:* ${phone || 'Não informado'}
• *Data/Hora:* ${now}`;
...
/**
 * Dispara notificação de FALHA NO CADASTRO DE USUÁRIO (Cliente / Fotógrafo / Produtor)
 */
export async function notifyRegistrationFailure({ role, name, email, phone, errorMessage, stack = '' }) {
  const now = new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' });
  const roleNames = {
  ...
  const roleLabel = roleNames[role] || role || 'Usuário';

  const aiDiagnosis = await analyzeErrorWithGemini({
    errorTitle: `Falha no Cadastro de ${roleLabel}: ${errorMessage}`,
    errorDetails: `Nome: ${name} | Email: ${email} | Erro: ${errorMessage}`,
    filename: 'services/api.ts (register)',
    stacktrace: stack
  });

  const message = `⚠️ *Alerta: Falha ao Cadastrar ${roleLabel}!*

• 👤 *Nome Tentado:* ${name || 'Não informado'}
• ✉️ *E-mail:* \`${email || 'Não informado'}\`
• 📱 *Telefone:* ${phone || 'Não informado'}
• ❌ *Erro do Sistema:* \`${errorMessage || 'Erro desconhecido'}\`

🤖 *Diagnóstico da IA:*
• *Causa:* ${aiDiagnosis.cause}
• *Ação Recomendada:* ${aiDiagnosis.fix}

⏰ *Data/Hora:* ${now}`;

  const buttons = [
    [
      { text: '👥 Ver Usuários no Painel', url: 'https://www.fotoclic.com.br/admin' },
      { text: '🌐 Abrir FotoClic', url: 'https://www.fotoclic.com.br' }
    ]
  ];
