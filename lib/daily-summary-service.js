...
export async function generateAndSendDailySummary() {
  ...
  try {
    ...
    // Taxa oficial negociada Appmax PIX: 0,99% + R$ 0,49 por transação
    const gatewayTotal = (totalRevenue * 0.0099) + (totalPhotosSold * 0.49);
    const photographerNetTotal = Math.max(0, totalRevenue - platformFee - gatewayTotal);
    ...
    const gatewayFormatted = gatewayTotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
    const photogNetFormatted = photographerNetTotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
    const topValFormatted = Math.max(0, topPhotographerTotal - (topPhotographerTotal * 0.06) - ((topPhotographerTotal * 0.0099) + (totalPhotosSold * 0.49))).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });

    const message = `📊 *Resumo Diário FotoClic • ${dateStr}*
⏰ *Horário:* 19:00 (Fechamento do Dia)

💵 *Extrato Financeiro Consolidado:*
• 🏷️ *Volume Bruto (VGV):* *${totalRevFormatted}* (${totalPhotosSold} ${totalPhotosSold === 1 ? 'venda' : 'vendas'})
• 💳 *Taxas Gateway Appmax (0,99% + R$ 0,49):* \`- ${gatewayFormatted}\`
• 📈 *Lucro Líquido FotoClic (6%):* \`+ ${feeFormatted}\`
• 💼 *Repasse aos Fotógrafos:* *${photogNetFormatted}*
• 🏆 *Destaque do Dia:* ${topPhotographerName} ${topPhotographerTotal > 0 ? `(${topValFormatted} líq.)` : ''}
