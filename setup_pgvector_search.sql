...
ALTER TABLE face_encodings 
ADD COLUMN descriptor vector(1024); -- 128 dimensions for face-api.js SSD MobileNet V1
...
-- 3. Create the matching function using L2 Distance
...
begin
  return query
  select subquery.id, subquery.photo_id, subquery.distance from (
    select
      face_encodings.id,
      face_encodings.photo_id,
      (face_encodings.descriptor <=> query_embedding) as distance
    from face_encodings
