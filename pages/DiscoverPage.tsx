...
const DiscoverPage: React.FC<DiscoverPageProps> = ({ onNavigate, initialSearch, onAddToCart, currentUser }) => {
  ...
  // Local state for the input to allow typing without immediate firing
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const [publicPhotographers, allCategories] = await Promise.all([
        ...
        if (searchTerm) {
          const results = await api.searchPhotos(searchTerm);
          setPhotos(results);
        } else {
          const allPhotos = await api.getAllPhotos(undefined, true, true);
          setPhotos(allPhotos);
        }
            ...
            <div className="relative group">
              ...
              <div className="relative flex items-center">
                <input
                  ...
                  className="w-full pl-14 pr-28 py-4 rounded-full bg-[#1A1A1A] border border-neutral-800 text-white placeholder-neutral-500 focus:outline-none focus:bg-[#222] focus:border-neutral-600 transition-all shadow-2xl text-lg"
                />
